...
const API_URL = 'http://10.109.253.112:1337/api'; // ✅ Mets ton IP locale ici
  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/utilisateurs?filters[Mail][$eq]=${encodeURIComponent(
          email
        )}&filters[Mot_de_passe][$eq]=${encodeURIComponent(password)}`
      );
      ...
      if (data.data && data.data.length > 0) {
        ...
        const utilisateur = raw.attributes
          ? { idUtilisateur: raw.id, ...raw.attributes }
          : raw;

        console.log('✅ Utilisateur connecté :', utilisateur);

        // ✅ On garde les infos principales
        await setUser({
        ...
        Alert.alert('Bienvenue', `Bonjour ${utilisateur.Nom || 'Utilisateur'} 👋`, [
          {
            text: 'Continuer',
            onPress: () => router.replace('/(tabs)/profil-utilisateur'),
          },
        ]);
      } else {
